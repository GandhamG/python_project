...
@pytest.mark.parametrize("token_type", ["Bearer"])
def test_use_authorization_header_when_authorization_bearer_header_is_provided(
    token_type, rf, staff_user, customer_user
):
    staff_access_token = create_access_token(staff_user)
    request = rf.request(
        HTTP_AUTHORIZATION_BEARER="ABC1234",
        HTTP_AUTHORIZATION=f"{token_type} {staff_access_token}",
    )
...
def test_use_default_header_as_a_first_try(rf, staff_user, customer_user):
    staff_access_token = create_access_token(staff_user)
