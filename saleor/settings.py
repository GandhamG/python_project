...
DATABASES = {
    DATABASE_CONNECTION_DEFAULT_NAME: dj_database_url.config(
        default="postgres://postgres:Gandham@123@localhost:5432/postgres",
        conn_max_age=600,
    ),
...
RETRY_COUNT = 10
BATCH_SIZE = 2
CLEAR_MULES_SOFT_LOG_DATE_RANGE = os.environ.get("CLEAR_MULES_SOFT_LOG_DATE_RANGE", 30)
CLEAR_SQS_LOG_DATE_RANGE = os.environ.get("CLEAR_SQS_LOG_DATE_RANGE", 60)
...
MIDDLEWARE = [
    ...
    "common.util.middleware.scgp_threadlocal_middleware.ThreadLocalMiddleware",
]
...
LOGGING = {
    ...
    "formatters": {
        ...
        "json": {
            ...
            "format": (
                "%(asctime)s %(request_id)s  %(levelname)s %(lineno)s %(message)s %(name)s "
...
ALLOWED_HOSTS = get_list(
    os.environ.get(
        "ALLOWED_HOSTS", "localhost,127.0.0.1,scgp-eordering-dev.scg-wedo.tech"
    )
)
# ALLOWED_HOSTS = ["*"]
...
# CELERY SETTINGS
AWS_CELERY_ACCESS_KEY = safequote(os.environ.get("AWS_CELERY_ACCESS_KEY") or "")
AWS_CELERY_SECRET_KEY = safequote(os.environ.get("AWS_CELERY_SECRET_KEY") or "")

AWS_REGION = os.environ.get("AWS_REGION")
QUEUE_URL = os.environ.get("QUEUE_URL")
endpoint = urlparse(QUEUE_URL)

QUEUE_ENDPOINT = f"{endpoint.netloc}{endpoint.path}"
CELERY_BROKER_URL = "sqs://%s:%s@" % (AWS_CELERY_ACCESS_KEY, AWS_CELERY_SECRET_KEY)
CELERY_TIMEZONE = TIME_ZONE

# These setting will be setup later on server through env file
CELERY_BROKER_TRANSPORT_OPTIONS = {
    "region": AWS_REGION,
    "predefined_queues": {
        # XXX: Queue with name 'celery' must be defined in 'predefined_queues'.
        "celery": {
            "url": QUEUE_URL,
            "access_key_id": AWS_CELERY_ACCESS_KEY,
            "secret_access_key": AWS_CELERY_SECRET_KEY,
        }
    },
}
...
# every one minute
PO_UPLOAD_SCHEDULE_CRONTAB_MINUTE = os.environ.get(
    "PO_UPLOAD_SCHEDULE_CRONTAB_MINUTE", "*/1"
)
# every one minute
EO_UPLOAD_SCHEDULE_CRONTAB_MINUTE = os.environ.get(
    "EO_UPLOAD_SCHEDULE_CRONTAB_MINUTE", "*/1"
)

CELERY_BEAT_SCHEDULE = {
    "map_gdc_users": {
        "task": "scgp_user_management.tasks.map_gdc_users",
        "schedule": crontab(hour=17, minute=0),  # At midnight
    },
    "remove_all_contract_order_drafts": {
        "task": "scg_checkout.tasks.remove_all_contract_order_drafts",
        "schedule": crontab(hour=0, minute=0),
    },
    "remove_all_failed_file_logs_at_midnight": {
        "task": "scgp_po_upload.tasks.delete_all_failed_file_logs",
        "schedule": crontab(hour=17, minute=0),
    },
    "eo_upload_send_email_summary": {
        "task": "scgp_export.tasks.eo_upload_send_email_summary",
        "schedule": crontab(hour=8, minute=0),  # Send once a day at 15:00
    },
    "clear_cart_at_midnight": {
        "task": "scgp_user_management.tasks.clear_cart",
        "schedule": crontab(hour=17, minute=0),
    },
    "sync_orders_data": {
        "task": "scg_checkout.tasks.sync_orders_data",
        "schedule": crontab(minute="*/1"),
    },
    "sync_orders_data_iplan": {
        "task": "scg_checkout.tasks.sync_orders_data_iplan",
        "schedule": crontab(minute="*/1"),
    },
    "task_sync_eo_upload": {
        "task": "scgp_eo_upload.tasks.task_sync_eo_upload",
        "schedule": crontab(minute=EO_UPLOAD_SCHEDULE_CRONTAB_MINUTE),
    },
    "send_emails_9am": {
        "task": "scgp_require_attention_items.tasks.send_email_via_r2_and_r4",
    ...
    "handle_po_upload": {
        "task": "scgp_po_upload.tasks.handle_po_upload",
        "schedule": crontab(minute=PO_UPLOAD_SCHEDULE_CRONTAB_MINUTE),
    },
    "DTR_DTP_logic": {
        "task": "sap_migration.tasks.DTR_DTP_logic",
        "schedule": crontab(minute="*/5"),
    },
    "r5_retry": {
        "task": "scgp_require_attention_items.tasks.r5_retry",
        "schedule": crontab(minute="*/1"),
    },
    "sqslog_cleanup": {
        "task": "scgp_require_attention_items.tasks.sqslog_cleanup",
        "schedule": crontab(hour=17, minute=0),  # Schedule at midnight BKK time,
    },
    "mulesoftlog_cleanup": {
        "task": "scgp_require_attention_items.tasks.mulesoftlog_cleanup",
        "schedule": crontab(hour=17, minute=0),  # Schedule at midnight BKK time,
    },
}

ENABLE_RETRY_R5 = get_bool_from_env("ENABLE_RETRY_R5", True)
if not ENABLE_RETRY_R5:
    warnings.warn("RETRY_R5 Job is disabled")
    CELERY_BEAT_SCHEDULE.pop("r5_retry")
...
# Can set maximum number of records by RELAY_CONNECTION_MAX_LIMIT
GRAPHENE = {
    "RELAY_CONNECTION_ENFORCE_FIRST_OR_LAST": True,
    "RELAY_CONNECTION_MAX_LIMIT": 500,  # default is 100
}
...
BUILTIN_PLUGINS = [
    ...
    "saleor.plugins.cognito_connect.plugin.CognitoConnectPlugin",
    "saleor.plugins.scg_send_email.plugin.ScgEmailPlugin",
    "saleor.plugins.scg_gdc_connect.plugin.ScgGdcPlugin",
    "saleor.plugins.scg_sap_client_api.plugin.SapAPIPlugin",
    "saleor.plugins.scg_iplan_api.plugin.IPlanAPIPlugin",
    "saleor.plugins.scg_sqs_po_upload.plugin.ScgSqsPoUploadPlugin",
    "saleor.plugins.scg_sns_po_upload.plugin.ScgSnsPoUploadPlugin",
    "saleor.plugins.scg_email_notification.plugin.ScgEmailNotificationPlugin",
    "saleor.plugins.scg_sns_eo_upload.plugin.ScgSnsEoUploadPlugin",
    "saleor.plugins.scg_sqs_delivery_gi.plugin.ScgSqsDeliveryGIPlugin",
    "saleor.plugins.scg_sqs_update_order.plugin.ScgSqsUpdateOrderPlugin",
    "saleor.plugins.scg_sqs_eo_upload.plugin.ScgSqsEoUploadPlugin",
    "saleor.plugins.scg_settings.plugin.ScgSettingsPlugin",
]
...
# Todo: Need to create ENV variable
...
JWT_TTL_APP_ACCESS = timedelta(
    seconds=parse(os.environ.get("JWT_TTL_APP_ACCESS", "30 minutes"))
)
...
SAP_ENV = os.environ.get("SAP_ENV", "DEV")

# MULESOFT SETTINGS: required
MULESOFT_API_URL = os.environ.get("MULESOFT_API_URL", None)
MULESOFT_CLIENT_ID = os.environ.get("MULESOFT_CLIENT_ID", None)
MULESOFT_CLIENT_SECRET = os.environ.get("MULESOFT_CLIENT_SECRET", None)
MULESOFT_API_TIMEOUT = int(os.environ.get("MULESOFT_API_TIMEOUT", "300"))
MULESOFT_LOG_URL = os.environ.get(
    "MULESOFT_LOG_URL", None
)  # ex. http://localhost:8000/api/log-mulesoft

if None in [MULESOFT_API_URL, MULESOFT_CLIENT_ID, MULESOFT_CLIENT_SECRET]:
    warnings.warn(
        "Make sure you've added values (env) for MULESOFT_API_URL, MULESOFT_CLIENT_ID, MULESOFT_CLIENT_SECRET."
    )

# TODO: improve this
# TODO: remove this
SNS_EO_DATA_ACCESS_KEY = ""
SNS_EO_DATA_SECRET_KEY = ""
SNS_EO_DATA_TOPIC_ARN = ""
SNS_EO_DATA_REGION_NAME = ""
if SAP_ENV == "UAT":
    SNS_EO_DATA_TOPIC_ARN = ""

FOREVER_TOKEN_FOR_EO_UPLOAD = os.environ.get("FOREVER_TOKEN_FOR_EO_UPLOAD", None)
CP_TOKEN_FOR_EOR = os.environ.get("CP_TOKEN_FOR_EOR", None)
INTERNAL_TOKEN_NAME = "SCG-Internal-Token"
INTERNAL_TOKEN = os.environ.get("INTERNAL_TOKEN", "scgdefaulttoken")
GDC_SECRET_KEY = os.environ.get("GDC_SECRET_KEY")
MULESOFT_API_LOGGING_VIA_THREAD_LOCAL_ENABLE = get_bool_from_env(
    "MULESOFT_API_LOGGING_VIA_THREAD_LOCAL_ENABLE", True
)
...
NEW_RELIC_E2E_METRIC_NAME = "Custom/E2E"
NEW_RELIC_MULESOFT_METRIC_NAME = "Custom/MulesoftAPI"
NEW_RELIC_DATETIME_FORMAT = "%Y-%m-%dT%H:%M:%S.%fZ"
NEW_RELIC_CREATE_ORDER_METRIC_NAME = "eor.e2e.createorder"
NEW_RELIC_CHANGE_ORDER_METRIC_NAME = "eor.e2e.changeorder"
NEW_RELIC_SPLIT_ORDER_METRIC_NAME = "eor.e2e.splitorder"
