...
from .cognito_connect.plugin import ExternalCognitoUser
...
class PluginsManager(PaymentInterface):
    ...
    def extenal_access_cognito(
        self, plugin_id: str, data: dict
    ) -> Optional["ExternalCognitoUser"]:
        """Obtain access tokens from authentication plugin."""
        default_value = None
    ...
    def scgp_po_upload_send_mail_when_call_api_fail(
        ...
        return self.__run_method_on_single_plugin(
            plugin,
            "scgp_po_upload_send_mail_when_call_api_fail",
            default_value,
    ...
    def scgp_send_order_confirmation_email(
        ...
        return self.__run_method_on_single_plugin(
            plugin,
            "scgp_send_order_confirmation_email",
            default_value,
            recipient_list=recipient_list,
            template_data=template_data,
            subject=subject,
    ...
    def scgp_user_check_valid_token_reset_password(
        self,
        plugin_id: str,
        email: str,
        token: str,
        ...
        return self.__run_method_on_single_plugin(
            plugin,
            "scgp_user_check_valid_token_reset_password",
            default_value,
            email,
            token,
        )

    def scgp_user_limit_time_out_reset_password(self, plugin_id: str):
        default_value = None
        plugin = self.get_plugin(plugin_id)
        return self.__run_method_on_single_plugin(
            plugin,
            "scgp_user_limit_time_out_reset_password",
            default_value,
        )

    def scgp_user_wrong_password_number(self, plugin_id: str):
        default_value = None
        plugin = self.get_plugin(plugin_id)
        return self.__run_method_on_single_plugin(
            plugin,
            "scgp_user_wrong_password_number",
            default_value,
        )

    def scgp_user_time_lock_user(self, plugin_id: str):
        default_value = None
        plugin = self.get_plugin(plugin_id)
        return self.__run_method_on_single_plugin(
            plugin,
            "scgp_user_time_lock_user",
            default_value,
        )

    def get_user_data_by_aduser(self, plugin_id: str, username: str):
        default_value = None
        plugin = self.get_plugin(plugin_id)
        return self.__run_method_on_single_plugin(
            plugin,
            "get_user_data_by_aduser",
            default_value,
            username,
        )

    def call_api_sap_client(
        self, plugin_id: str, param: str, method: str, data: dict, data_example: dict
    ):
        default_value = None
        plugin = self.get_plugin(plugin_id)
        return self.__run_method_on_single_plugin(
    ...
    def get_require_attention_url(self, plugin_id: str):
        default_value = None
        plugin = self.get_plugin(plugin_id)
        return self.__run_method_on_single_plugin(
            plugin,
            "get_require_attention_url",
            default_value,
        )

    def get_list_email_eo_upload_fail_case(self):
        default_value = []
        plugin = self.get_plugin("scg.email_notification")
        return self.__run_method_on_single_plugin(
            plugin,
            "get_list_email_eo_upload_fail_case",
            default_value,
        )

    def get_list_email_eo_upload_summary(self):
        default_value = []
        plugin = self.get_plugin("scg.email_notification")
        return self.__run_method_on_single_plugin(
            plugin,
            "get_list_email_eo_upload_summary",
            default_value,
        )
    ...
    def scgp_send_email_with_excel_attachment(
        ...
        return self.__run_method_on_single_plugin(
            plugin,
            "scgp_send_email_with_excel_attachment",
            default_value,
            recipient_list=recipient_list,
            template_data=template_data,
            subject=subject,
