...
def test_permission_group_create_mutation(
    ...
    group = Group.objects.get(name="New permission group")
...
def test_permission_group_update_mutation_only_permissions(
    ...
    group = Group.objects.get(name="Manage user groups.")
    assert group.name == old_group_name
    assert permission_group_data["name"] == group.name
    permissions = {
        permission["name"] for permission in permission_group_data["permissions"]
    ...
    expected_errors = [
        {
            ...
            "permissions": [OrderPermissions.MANAGE_ORDERS.codename.upper()],
            "users": None,
        },
        {
...
# default two SCGP Groups
@pytest.mark.parametrize(
    "permission_group_filter, count",
    (({"search": "Manage user groups"}, 1), ({"search": "Manage"}, 2), ({}, 5)),
)
def test_permission_groups_query(
    ...
    Group.objects.get_or_create(name="SCGP Customer")
    Group.objects.get_or_create(name="SCGP Admin")
...
# SCGP groups are two default groups
@pytest.mark.parametrize(
    "permission_group_sort, result",
    (
        (
            {"field": "NAME", "direction": "ASC"},
            ["Add", "Manage user groups.", "Remove", "SCGP Admin", "SCGP Customer"],
        ),
        (
            {"field": "NAME", "direction": "DESC"},
            ["SCGP Customer", "SCGP Admin", "Remove", "Manage user groups.", "Add"],
        ),
    ),
)
