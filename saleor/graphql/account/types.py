...
class Address(ModelObjectType):
    ...
    def resolve_is_default_shipping_address(root: models.Address, _info):
        ...
        user_default_shipping_address_pk = getattr(  # NOQA
    ...
    def resolve_is_default_billing_address(root: models.Address, _info):
        ...
        user_default_billing_address_pk = getattr(  # NOQA
...
class Division(ModelObjectType):
    id = graphene.GlobalID(required=True)
    name = graphene.String(required=True)
    company = graphene.Field(Company)
...
class User(ModelObjectType):
    ...
    company = graphene.Field(Company)
    distribution_channel = graphene.Field(Channel)
    division = graphene.Field(Division)
    office = graphene.Field(Office)
    ...
    @staticmethod
    def resolve_display_name(root: models.User, info):
        display_name = ScgpUser.objects.filter(user_id=root.id).values("display_name").first()
