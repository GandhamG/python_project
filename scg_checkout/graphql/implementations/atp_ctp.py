...
from sap_migration.models import OrderLines, Order, OrderLineIPlan
...
def change_order_call_atp_ctp_request(manager, lines_input):
    order_lines = OrderLines.all_objects.select_related("material_variant", "order", "iplan").filter(
        id__in=[line.get("line_id") for line in lines_input]
    )
    order = Order.objects.annotate(sale_org_code=F("sales_organization__code"),
                                   sale_group_code=F("sales_group__code"),
                                   sold_to__sold_to_code=F("sold_to__sold_to_code")
                                   ).get(id=order_lines[0].order.id)

    # call rollback first
    is_full_iplan_success, i_plan_error_messages, iplan_response = _call_iplan_rollback_if_exists_response(manager,
                                                                                                           order,
                                                                                                           order_lines)
    ...
    # call iplan request
    line_inputs = _insert_iplan_response_lines_to_line_input(manager, order, order_lines, lines_input)
        ...
        for response_line in line_input["iplan_response_lines"]:
            ...
            if response_line["returnStatus"].lower() == "success":
                ...
                rs_item.update({
                    ...
                    "confirm_date": response_line.get("dispatchDate") or mock_confirm_date(
                        line_input.get("request_date"),
                        response_line.get("status")
                    ),
    ...
    return _transform_data_to_return(success_items), i_plan_messages, _transform_data_to_return(failed_items)
...
def _call_iplan_rollback_if_exists_response(manager, order, order_lines):
    ...
    if rollback_lines_input:
        is_full_iplan_success, i_plan_error_messages, iplan_response = _call_iplan_rollback(manager,
                                                                                            rollback_lines_input)
...
def _insert_iplan_response_lines_to_line_input(manager, order, order_lines, _lines_input):
    ...
    response_lines = response.get("DDQResponse", {}).get("DDQResponseHeader")[0].get("DDQResponseLine", [])

    line_id_to_order_line = {str(order_line.id): order_line for order_line in order_lines}
        ...
        line_params.update(
            {
                ...
                "consignmentLocation": line_input.get("consignment_location","").lstrip('0') or order.sale_group_code,
...
def _build_request_body_for_iplan_confirm(input_items, es21_response=None):
    ...
    for input_item in input_items:
        ...
        item_no_to_es21_order_schedules_out = {item.get("itemNo").lstrip("0"): item for item in
                                               es21_order_schedules_out}
        on_hand_stock = input_item.get("on_hand_stock")
        es21_order_schedules_out = item_no_to_es21_order_schedules_out[calculated_item_no]

        confirm_quantity = str(es21_order_schedules_out.get("confirmQuantity")) if on_hand_stock else "0"
...
def change_order_call_atp_ctp_confirm(info, input_data):
    ...
    order_lines = OrderLines.all_objects.select_related("material_variant", "order", "iplan").filter(
        id__in=[item.get("line_id") for item in input_items]
    )
    _inject_order_line_instance_to_input_item(input_items, order_lines)
    _inject_calculated_item_no_to_input_items(order_lines[0].order, order_lines, input_items)
    ...
    if action_type == ATPCTPActionType.ROLLBACK:
        # call iplan rollback
        is_full_iplan_success, i_plan_error_messages, _ = _call_iplan_rollback(info, input_items)
        logging.info(f"[Domestic: ATP/CTP RollBack] i_plan_roll_back_error_messages: {i_plan_error_messages},"
                     f" is_full_iplan_success : {is_full_iplan_success}")
        if is_full_iplan_success:
            _delete_iplan_response([order_line.iplan for order_line in order_lines])
            input_items = rollback_input_items(input_items)
        return sap_order_error_messages, sap_item_messages, i_plan_error_messages, input_items

    # COMMIT CASE
    order = Order.objects.filter(so_no=input_items[0]["order_no"]).first()

    # call es21 update order
    is_es21_success, sap_order_error_messages, sap_item_messages, es21_response = _call_es_21(info, order, input_items)
    logging.info(f"[Domestic: ATP/CTP Commit] is_es21_success: {is_es21_success},"
                 f" sap_order_error_messages : {sap_order_error_messages},"
                 f"sap_item_messages: {sap_item_messages}")
    if not is_es21_success:
        # call iplan rollback
        is_full_iplan_success, i_plan_error_messages, _ = _call_iplan_rollback(info, input_items, order=order)
        logging.info(f"[Domestic: ATP/CTP Accept] i_plan_roll_back_error_messages: {i_plan_error_messages},"
                     f" is_full_iplan_success : {is_full_iplan_success}")
        if is_full_iplan_success:
            _delete_iplan_response([order_line.iplan for order_line in order_lines])
        return sap_order_error_messages, sap_item_messages, i_plan_error_messages, input_items
    ...
    # call iplan commit
    _, i_plan_error_messages, iplan_response = _call_iplan_confirm(info, input_items, es21_response, order=order)
    logging.info(f"[Domestic: ATP/CTP Confirm] i_plan_confirm_error_messages: {i_plan_error_messages}")
...
def _inject_order_line_instance_to_input_item(input_items, order_lines):
    ...
    for input_item in input_items:
        input_item["order_line_instance"] = str_line_id_to_order_line.get(input_item.get("line_id"))
...
def _call_es_21(info, order, input_items):
    ...
    return is_es21_success, sap_order_error_messages, sap_item_messages, es21_response


def _call_iplan_confirm(info, input_items, es21_response=None, order=None):
    iplan_confirm_body = _build_request_body_for_iplan_confirm(input_items, es21_response)
    iplan_response = call_i_plan_confirm(info, iplan_confirm_body, order=order)
    is_full_iplan_success, i_plan_error_messages = get_iplan_error_messages(iplan_response)
    return is_full_iplan_success, i_plan_error_messages, iplan_response


def _call_iplan_rollback(info, input_items, order=None):
    request_body = _build_request_body_for_iplan_rollback(input_items)
    iplan_response = call_i_plan_confirm(info, request_body, order=order)
    is_full_iplan_success, i_plan_error_messages = get_iplan_error_messages(iplan_response)
    return is_full_iplan_success, i_plan_error_messages, iplan_response
...
def _build_request_body_for_es_21(order, input_items):
    ...
    for input_item in input_items:
        ...
        if order_line.ship_to:
            order_partners.append(
                {
                    "partnerRole": "WE",
                    "partnerNumb": order_line.ship_to.split(" - ")[0],
        ...
        order_items_inx = {
            ...
            "targetQty": is_new_order_line or (order_line.quantity != param_quantity),
            "plant": is_new_order_line or (order_line.plant != input_item.get("plant")),
        }
        ...
        param_order_schedules_in_x.append({
            ...
            "updateflag": "I" if is_new_order_line else "U",
            "scheduleLine": "0001",
            "requestDate": is_new_order_line or (param_req_date != original_param_req_date),
            "requestQuantity": is_new_order_line or (param_request_quantity != order_line.quantity),
    ...
    if order_partners:
        body["orderPartners"] = order_partners
...
def _set_order_line_fields(order_line, iplan, input_item):
    ...
    order_line.request_date = DateHelper.iso_str_to_obj(input_item.get("original_request_date"))
    ...
    order_line.i_plan_on_hand_stock = input_item.get("on_hand_stock")  # this one used in SEO-1181
    order_line.i_plan_operations = {"blockCode": input_item.get("block_code")}  # this one used in SEO-1181

    set_status_for_order_line(order_line)


def _bulk_update_iplans(iplans):
    OrderLineIPlan.objects.bulk_update(iplans, fields=[
        "item_no",
        "item_status",
...
def _bulk_update_order_lines(order_lines):
    OrderLines.objects.bulk_update(order_lines, fields=[
        "item_no",
        "request_date",
        "quantity",
        "return_status",
